...
def run_demo(
    ...
    op = PhotocurrentMapOp(J=J, subsampler=subsampler, device=device)
    y_subsampled_tensor = op(im_tensor)
    # im_reconstructed_tensor = op.adjoint(y_subsampled_tensor)
    im_reconstructed_tensor = op.pseudo_inv(y_subsampled_tensor)
    ...
    sigma = 0.1  # noise level for denoiser

    # Can you help me write the mathematical description and references for the docstring using rst format?

    def denoiser_fn_admm(x: torch.Tensor) -> torch.Tensor:
        with torch.no_grad():
            return (
                denoiser(x.unsqueeze(0).unsqueeze(0), sigma=sigma).squeeze(0).squeeze(0)
            )
    ...
    pnp = PnP(physics=op, denoiser=denoiser_fn_admm, algorithm="ADMM")
